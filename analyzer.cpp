...
// 実行ファイル用のグローバル変数の配置終了アドレス (コード領域の上端の直後．ここから下へ順次割り当てる)
static const int g_bin_global_end_addr = 0x10000;
...
std::map<std::string, const symbol_t *> Analyzer::operator()() {
    ...
    // (ROM用ではスタックはメモリの上端から下へ伸びるため，残りがなければ関数を1つも呼び出せない．
    //  実行ファイル用でも，メモリ全体に収まらないグローバル変数は置けないため同じく検査する．

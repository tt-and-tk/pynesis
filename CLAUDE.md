...
- `c2bin.exe`: `-pn` `-pt` `-bin` の3つ全てを指定する(自動導出はサポートしない．`-pt`省略時は1段階目の`compile_c_to_asm`が`.pn`から`-pt`を自動導出して中間アセンブリファイルを実際に書き出してしまい，2段階目の`assemble_asm_to_sv`が改めて引数を解析した時点で`-pt`未指定としてエラーになる．エラーになる前に中間ファイル生成という副作用が発生する点に注意)

...
// ハードウェア制約: 1命令がメモリ上に占めるバイト数 (実行ファイル用の命令列の大きさを求めるのに使う)
const int INSTRUCTION_BYTES = 8;
